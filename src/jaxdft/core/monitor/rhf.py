...
def e_nn(z_list, pos_list):
    ...
    """
    return jnp.sum(
        jnp.triu(jnp.outer(z_list, z_list), k=1)
        / jnp.linalg.norm(pos_list[:, None, :] - pos_list[None, :, :], axis=-1)
    ).sum()


@jax.jit
def coef_norm_func(coef1, coef2):
    return jnp.outer(coef1, coef2)
...
def Sxyz_func(l1, l2, norm=True):
    ...
    item_Sxyz_func = _Sxyz_func(l1[0], l1[1], l1[2], l2[0], l2[1], l2[2], norm=norm)

    item_batch_Sxyz_func = jax.vmap(item_Sxyz_func, in_axes=(0, 0, None, None))
    ...
    def Sxyz_contraction(a, b, coef1, coef2, pa_xyz, pb_xyz):
        ...
        a_shape = a.shape
        b_shape = b.shape
        a = jnp.outer(a, jnp.ones(b_shape)).flatten()
        b = jnp.outer(jnp.ones(a_shape), b).flatten()

        result = jnp.sum(
            coef_norm_func(coef1, coef2).flatten()
            * item_batch_Sxyz_func(a, b, pa_xyz, pb_xyz)
        )
        return result
...
def int_overlap_and_kinetic_funcs(ao_list):
    ...
    for k in combinations_dict.keys():
        funcs_S[k] = jax.vmap(Sxyz_func(k[0], k[1]), in_axes=(0, 0, 0, 0, 0, 0))
        ...
        #     )
        # )
    return combinations_dict, funcs_S


def run_overlap_funcs(ao_list, combinations_dict, funcs_s):
    expon = jnp.array([ao.expon for ao in ao_list])
    coef = jnp.array([ao.coef for ao in ao_list])
    l = [ao.l for ao in ao_list]
    r = jnp.array([ao.r for ao in ao_list])
...
if __name__ == "__main__":
    ...
    NH3 = Atoms(
        numbers=[7, 1, 1, 1],
        positions=[
            [0.30926, 0.30926, 0.30926],
            [2.17510, 0.0, 0.0],
            [0.0, 2.17510, 0.0],
            [0.0, 0.0, 2.17510],
        ],
    )
    basis_dict = get_basis("sto-3g", ["N", "H"])
    # basis_dict = {
    #     '1': [
    #         {'angular_momentum': 0,
    ...
    # timeit
    combinations, func_S = int_overlap_and_kinetic_funcs(ao_list)
    S = run_overlap_funcs(ao_list, combinations, func_S)
    st_time = time.time()
    for _ in range(1):
        S = run_overlap_funcs(ao_list, combinations, func_S)
    end_time = time.time()
