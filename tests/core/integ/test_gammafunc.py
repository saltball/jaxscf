import pytest
import numpy as np
from jaxdft.core.integ.gammafunc import Fm
import jax
...
def test_gamma_w_0(m):
    result = Fm(m, 0.)
    assert result == 1/(2*m+1)
...
@pytest.mark.parametrize(
    "w",
    [
        0., 0.1, 0.5, 1.0, 2.0, 16., 20.,
    ],
)
def test_gamma_Pade(m, w):
    r"""
    Pade approximate Boys function Used by Xv, Guangxian.
    徐光宪; 黎乐民; 王德民. 量子化学：基本原理和从头计算法, 第二版.; 科学出版社: 北京, 2007. C.2 p.67
    """
    result = Fm(m, w)
    expected_result = FmDefold(m, w)
    assert np.allclose(result, expected_result, atol=1e-9)
